...
import com.jizhi.hududu.uclient.widget.ServiceHansWasingDialog;
import com.jizhi.hududu.uclient.widget.ServiceHansWasingDialog.CallBackWasingInteface;
 ...
 * 
 * @author Xuj
 * @date 2015年8月17日 16:24:51
 */
public class HandWasingMainActivity extends Activity implements OnClickListener {
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		setContentView(R.layout.hand_wasing_main);
	...
	public void reaTop(View view) {
		Intent intent = new Intent(HandWasingMainActivity.this,
	...
	public void initData() {
		title.setText(R.string.main_chloth);
		...
		poiInfos = (List<NearByPoiInfo>) getIntent().getSerializableExtra("poiInfo");
		...
		currentHouse.setWorking_hours("1.0");
		currentHouse.setFinalPrice(currentHouse.getPrice() * 1);
		cost_value.setText("￥" + DataForMat.twoDecimalPlaces(currentHouse.getFinalPrice()));
		service_time_value.setText("1.0小时");
	...
	public void onClick(View view) {
		...
		case R.id.door_service_date_relative:
			ServiceDateDialog dialog = new ServiceDateDialog(HandWasingMainActivity.this);
			dialog.show();
			dialog.setListener(new ServiceDateInterface() {
				...
				public void onClick(String chooseDate) {
					try {
						...
						CommonMethod.makeNoticeShort(HandWasingMainActivity.this, "日期解析出错!");
						e.printStackTrace();
						return;
					}
		...
		case R.id.service_time_relative:
			ServiceHansWasingDialog dialog1 = new ServiceHansWasingDialog(
					HandWasingMainActivity.this);
			dialog1.show();
			dialog1.setListener(new CallBackWasingInteface() {
				@Override
		...
		case R.id.submit:
			...
			if (time == null) {
				CommonMethod.makeNoticeShort(HandWasingMainActivity.this,
						"请选择上门服务时间!");
				return;
			}
	...
	public List<NameValuePair> params(HouseholdCleaningBean bean) {
		List<NameValuePair> params = new ArrayList<NameValuePair>();
		params.add(new BasicNameValuePair("cid", (String) SPUtils.get(this,"mobile", "", Constance.HUDUDUUSER))); // 顾客ID
		params.add(new BasicNameValuePair("jobdesc", bean.getJob_desc() + ""));
		params.add(new BasicNameValuePair("servertime", bean.getServer_time())); // 上门服务的时间
		params.add(new BasicNameValuePair("address", bean.getAddress())); // 服务地址
		params.add(new BasicNameValuePair("location", bean.getLocation())); // 服务地点坐标
		params.add(new BasicNameValuePair("workinghours", bean.getWorking_hours())); // 工作时长 以小时为一个单位
	...
	public List<NameValuePair> cash_params() {
		...
		params.add(new BasicNameValuePair("wtype",CMD.WH)); //类型
		return params;
	}
	...
	public void submitHouseholdClean() {
		...
		dialog = CustomProgress.show(HandWasingMainActivity.this,
		...
		new Thread(new Runnable() {
			...
			public void run() {
				final HouseHoldCleaningState state = resolution.resolution(
						HandWasingMainActivity.this,
						CMD.SUBMIT_WASHCLOSE_CLEANING, params(currentHouse));
				runOnUiThread(new Runnable() {
					@Override
					public void run() {
						if (state == null) {
							CommonMethod.makeNoticeShort(
									HandWasingMainActivity.this, "数据解析出错!");
						}
						if (state.getState() == 1) {
							CommonMethod.makeNoticeShort(
									HandWasingMainActivity.this, "提交成功!");
							...
							CommonMethod.makeNoticeShort(
									HandWasingMainActivity.this, "提交失败!:"
											+ state.getCode());
						}
						dialog.dismiss();
					}
	...
	public void searchCash_coupon() {
		new Thread(new Runnable() {
			...
			public void run() {
				...
				final CashCouponState cash_state = resolution.search_cash_coupons(HandWasingMainActivity.this,CMD.SEARCH_CASH_PRICE,cash_params());
				runOnUiThread(new Runnable() {
					@Override
					public void run() {
						if (cash_state == null) {
